...
| **F9** | ✅ done | **Demo product-polish (ADR-018).** Friendlier `/demo`: **preset example buttons** (healthy/failing) + units/tooltips + bounded inputs + a **risk-meter** result (fixes "too technical"); **light/dark theme** (theme-aware, keeps the no-CDN constraint); **EN/PT-BR i18n** of the UI shell. A front-end/product-showcase play, **paired with `receivables-agent` Phase 9** for a shared design language. Honesty banner + reported-number framing intact in both themes/languages. |

### Epoch 2 (F10–F17) — per-vehicle, multi-label, interactive → full plan in [`EPOCH2_PLAN.md`](EPOCH2_PLAN.md)
...
| **F16** | ☐ planned — **◄ NEXT** | **Kubernetes (`kind`, local, $0)** — orchestrate the multi-service system. **Closes the K8s gate.** Needs **F14a**. Justified by the **market**, not by operational need — Cloud Run stays the production answer; say so out loud. `kind` is local ⇒ never claim "operated a cluster in production." |
| **F17** | ☑ **DONE** (2026-07-14, ADR-027) — **IaC gate CLOSED** | **IaC / Terraform** — codify the Cloud Run + generation job + Neon (+ bucket) deploy. **Closes the IaC gate.** Needs **F14a**. Justified by the **project**: F7 shipped imperative (`deploy_cloudrun_neon.sh` ran once; no source of truth to diff/recreate/destroy). Terraform knows the *resources* match — **not** that the app works. |
